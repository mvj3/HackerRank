...
"""
Recursive Digit Sum
Your Recursive Digit Sum submission got 30.00 points.  
...
def superDigit_iterative(n, k):
    while len(n) >= 2:
        new_n = sum(list(map(int, list(n)))) * k
        k = 1
        n = str(new_n)
    return n
...
def superDigit_recursive(n, k):
    ...
    if len(n) >= 2:
        new_n = sum(list(map(int, list(n)))) * k
        return superDigit(str(new_n), 1)
    else:
        return n            
superDigit = superDigit_recursive
