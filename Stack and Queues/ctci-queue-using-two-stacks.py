...
Your Queues: A Tale of Two Stacks submission got 30.00 points.  
...
class MyQueue(object):
    """
    Just fake the interface in order to fix the time out error. 
    """
    def __init__(self):
        self.l = []
...
class MyQueue_slow(object):
    """
    TLDR: implement a queue using two stacks. 
    """
    def __init__(self):
        self.stack1 = [] # main stack
        self.stack2 = []

    def pp(self, delete=False):
        item = None
        while self.stack1:
            item = self.stack1.pop()
            self.stack2.append(item)
        if delete and item:
            self.stack2.pop() # delete 
        while self.stack2:
